...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
	...
	<body class="boxed-bg">
      ...
      <div>
        <form action="/v1/api/signup" method="POST" class="auth-shadow bg-[#174e90] w-[450px] rounded-md p-8 space-y-6">
          @csrf
          ...
          <div class="w-full flex flex-col items-center">
            ...
            @error('g-recaptcha-response')<span class="text-xs text-error">{{ $message }}</span>@enderror
            @error('captcha')<span class="text-xs text-error">{{ $message }}</span>@enderror
          </div>

          <div class="w-full flex flex-col justify-center items-center gap-3">
            <x-button class="w-full font-semibold" type="submit" primary label="Signup" rounded="2xl" />
            <!-- Google Login -->
