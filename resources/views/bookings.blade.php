...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <script>
      ...
      function approveEvent(eventID) {
        const url = `/v1/api/booking/${eventID}/approve`
    ...
    <div class="flex">
      <aside class="shrink-0 space-y-8 w-[400px] bg-neutral-content/50 border-r-2 border-neutral-content p-6">
        <h1 class="text-2xl">Pending Bookings</h1>
        <div class="space-y-2">
          @foreach ($pendingEvents as $pendingEvent)
          <div id="{{ $pendingEvent->id }}" class="p-2 px-4 border border-neutral-content rounded-md shadow-md">
            <div class="flex justify-between">
              <div class="flex items-center gap-2">
                <div class="size-2 rounded-full bg-[#fcba03]"></div>
                <p>{{ $pendingEvent->title }}</p>
              </div>
              <x-button onclick="approveEvent({{ $pendingEvent->id }})" type="button" primary flat label="Approve" />
            </div>

            <div class="w-full border-b-2 border-neutral-content h-1 mb-2"></div>

            <div class="space-y-2 text-sm text-base-content/50">
              <div class="flex items-center gap-2">
                <x-icon name="user-circle" />
                <div>
                  <p>{{ $pendingEvent->user->name }}</p>
                  <p>{{ $pendingEvent->user->email }}</p>
                </div>
              </div>

              <div class="flex items-center gap-2">
                <x-icon name="calendar" />
                <p>{{ $pendingEvent->start }} - {{ $pendingEvent->end }}</p>
              </div>
              <div class="flex items-center gap-2">
